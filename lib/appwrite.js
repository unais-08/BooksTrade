...
export const config = {
  ...
  tradeCollectionId: APPWRITE_TRADE_COLLECTION_ID,
};
console.log(config.messageCollectionId);
...
export async function getFilePreview(fileId, type) {
  ...
  try {
    if (type === "video") {
      // Ensure config.storageId is correct for your setup
      fileUrl = storage.getFileView(config.storageId, fileId);
    } else if (type === "image") {
      // Use getFileView for images to get the direct file URL
      fileUrl = storage.getFileView(config.storageId, fileId);
    } else {
    ...
    console.error("Error in getFilePreview:", error); // Log the specific error
...
export const subscribeToMessages = (callback) => {
  const unsubscribe = client.subscribe(
    ...
    (response) => {
      ...
      console.log(
        "Received real-time response:",
        response.payload.senderName,
        "send a message :",
...
//trading functions start from here......
export const createTradeRequest = async (
  ...
  try {
    const response = await databases.createDocument(
      config.databaseId,
      config.tradeCollectionId,
      "unique()", // Generate unique ID for the request
      tradeData
    );
    return response;
  } catch (error) {
...
export const fetchTradeRequests = async (ownerName) => {
  try {
    const response = await databases.listDocuments(
      config.databaseId,
      config.tradeCollectionId,
      [
        Query.equal("ownerName", ownerName), // Fetching trade requests for the uploader by ownerName
      ]
    );

    return response.documents; // Return the list of trade requests for this uploader
  } catch (error) {
    console.log("Error fetching trade requests:", error);
    return []; // Return an empty array on error
  }
};

export const handleUpdateStatus = async (documentId, bookID, newStatus) => {
  try {
    // Step 1: Update the specific request to "Accepted"
    ...
    console.log("Trade request status updated:", response);

    // Step 2: If the new status is "Accepted", reject other pending requests for the same book
    if (newStatus === "Accepted") {
      const otherRequests = await databases.listDocuments(
        config.databaseId,
        config.tradeCollectionId,
        [
          Query.equal("bookID", bookID), // Match requests for the same book
          Query.equal("status", "Pending"), // Only pending requests
        ]
      );

      // Step 3: Reject each pending request for the same book
      const updatePromises = otherRequests.documents.map((request) =>
      ...
      await Promise.all(updatePromises); // Await all update operations
      Alert.alert(
        "Request Update",
        `The request has been ${newStatus.toLowerCase()}.`,
        [{ text: "OK" }]
      );
      console.log(
        `All other pending requests for book ${bookID} have been rejected.`
      );
    }
  } catch (error) {
